...
resource "aws_secretsmanager_secret" "db_credentials" {
  name                    = "devforge-aurora-credentials-${random_id.suffix.hex}"
  recovery_window_in_days = 0  # For development - immediate deletion
  # Tags skipped: aiengineer lacks secretsmanager:TagResource (see 1_permissions notes)
}
...
resource "aws_db_subnet_group" "aurora" {
  name       = "devforge-aurora-subnet-group"
...
resource "aws_security_group" "aurora" {
  name        = "devforge-aurora-sg"
  description = "Security group for DevForge Aurora cluster"
...
resource "aws_rds_cluster" "aurora" {
  cluster_identifier     = "devforge-aurora-cluster"
  engine                 = "aurora-postgresql"
  engine_mode            = "provisioned"
  engine_version         = "15.12"
  database_name          = "devforge"
  master_username        = "devforgeadmin"
...
resource "aws_rds_cluster_instance" "aurora" {
  identifier          = "devforge-aurora-instance-1"
...
resource "aws_iam_role" "lambda_aurora_role" {
  name = "devforge-lambda-aurora-role"
...
resource "aws_iam_role_policy" "lambda_aurora_policy" {
  name = "devforge-lambda-aurora-policy"
