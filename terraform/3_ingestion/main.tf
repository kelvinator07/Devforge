...
resource "aws_s3_bucket" "vectors" {
  bucket = "devforge-vectors-${data.aws_caller_identity.current.account_id}"
...
resource "aws_iam_role" "lambda_role" {
  name = "devforge-ingest-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "devforge-ingest-lambda-policy"
...
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/devforge-ingest"
  retention_in_days = 7
...
resource "aws_api_gateway_rest_api" "api" {
  name        = "devforge-api"
  description = "DevForge Platform API"
