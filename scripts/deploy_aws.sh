...
#   ./scripts/deploy_aws.sh database      # just 3_database + run migrations
#   ./scripts/deploy_aws.sh worker        # build + push worker image + apply 4_worker
#   ./scripts/deploy_aws.sh control-plane # build + push cp image + apply 5_control_plane
...
case "$cmd" in
  ...
  worker)
    ...
    # from S3 Vectors. Pulled from the existing 3_database state and
    # derived from the AWS account id.
    export TF_VAR_vector_bucket_name="devforge-vectors-${ACCOUNT}"
    export TF_VAR_aurora_cluster_arn=$(cd terraform/3_database && terraform output -raw aurora_cluster_arn)
    export TF_VAR_aurora_secret_arn=$(cd terraform/3_database && terraform output -raw aurora_secret_arn)
    ...
    # If 5_control_plane has been deployed, pre-populate the worker task def
    # with its URL too (helps standalone `aws ecs run-task` smoke tests).
    if [ -f terraform/5_control_plane/terraform.tfstate ]; then
      export TF_VAR_control_plane_api=$(cd terraform/5_control_plane && terraform output -raw api_endpoint 2>/dev/null || echo "")
    fi
    tf_apply terraform/4_worker
    ;;
  ...
  control-plane)
    ...
    # current values from 3_database state to keep tfvars from going stale.
    export TF_VAR_aurora_cluster_arn=$(cd terraform/3_database && terraform output -raw aurora_cluster_arn)
    export TF_VAR_aurora_secret_arn=$(cd terraform/3_database && terraform output -raw aurora_secret_arn)
    ...
    # default), then auto-append the CloudFront site URL when 6_frontend
    ...
    if [ -f terraform/6_frontend/terraform.tfstate ]; then
      FRONTEND_URL=$(cd terraform/6_frontend && terraform output -raw site_url 2>/dev/null || echo "")
      if [ -n "$FRONTEND_URL" ] && [[ "$CORS_BASE" != *"$FRONTEND_URL"* ]]; then
        CORS_BASE="$CORS_BASE,$FRONTEND_URL"
      fi
    fi
    export TF_VAR_cors_origins="$CORS_BASE"
    tf_apply terraform/5_control_plane
  ...
  frontend)
    ...
    tf_apply terraform/6_frontend
    BUCKET=$(cd terraform/6_frontend && terraform output -raw bucket_name)
    DIST=$(cd terraform/6_frontend && terraform output -raw cloudfront_distribution_id)
    SITE=$(cd terraform/6_frontend && terraform output -raw site_url)
  ...
  destroy)
    tf_destroy terraform/6_frontend || true
    tf_destroy terraform/5_control_plane || true
    tf_destroy terraform/3_database || true
    tf_destroy terraform/4_worker || true
